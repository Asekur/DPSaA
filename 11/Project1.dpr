...
begin
  ...
  writeln('Ввведите количество элементов в первой очереди:');
  readln(count);
  st := nil;
  PFirstTask := nil;
  Psecondtask := nil;
  writeln('Ввведите элементы первой очереди:');
  for i := 1 to count do
  begin
    readln(buf.id);
  ...
  writeln;
  writeln('Ввведите количество элементов во второй очереди: ');
  readln(count);
  writeln('Ввведите элементы второй очереди:');
  for i := 1 to count do
  begin
    readln(buf.id);
  ...
  writeln;
  writeln('Полученная очередь (в обратном порядке):');
  while (st<>nil) do
  begin
    write(pop(st):4);
  end;
