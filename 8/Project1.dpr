...
begin
  ...
  level:= 1;
  tree:= nil;
  writeln('Введите элементы дерева. Если хотите закончить, введите пустую строку.');
  repeat
    readln(vvod);
    if (vvod <> '') and not(vvod[1] in ['A'..'z']) then
    begin
      inputInt:= strtoint(vvod);
      addToTree(tree,inputInt,level);
      if level > maxLevel then
        maxLevel:= level;
    end;
    if (vvod <> '') and (vvod[1] in ['A'..'z']) then
      writeln ('Введите численное значение');
  until (vvod = '');
end;
